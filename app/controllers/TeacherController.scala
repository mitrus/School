...
object TeacherController extends Controller with MongoController with AuthElement with AuthConfigImpl {
  ...
  def becomeMaster = AsyncStack(AuthorityKey -> TeacherPermission) { implicit request =>
    becomeMasterForm.bindFromRequest().fold(
      formWithErrors => Future.successful(BadRequest(views.html.masterPage("Error in form!", loggedIn, loggedIn.user.asInstanceOf[Teacher]))),
      data => {
        loggedIn.user.asInstanceOf[Teacher].master match {
          case None => {
            ...
            secretKeyForForm.flatMap {
              case None => {
                ...
                insertSK.flatMap { _ =>
                  val newAccount = loggedIn.copy(user = loggedIn.user.asInstanceOf[Teacher].copy(master = Some(data.form)))
                  usersDB.update(BSONDocument("id" -> loggedIn.id), newAccount).map {x =>
                    Ok(views.html.masterPage("Secret key " + key + " has been added!", loggedIn, loggedIn.user.asInstanceOf[Teacher]))
                  }
                } recover {
                  case _ => Ok(views.html.masterPage("Error in generation secret key!", loggedIn, loggedIn.user.asInstanceOf[Teacher]))
                }
              }
              case _ => {
                Future.successful {Ok(views.html.masterPage("This form's alread a master!", loggedIn, loggedIn.user.asInstanceOf[Teacher]))}
              }
            }
          }
          case _ => {
            Future.successful {Ok(views.html.masterPage("You're already a master!", loggedIn, loggedIn.user.asInstanceOf[Teacher]))}
          }
        }
  ...
  def masterPage = StackAction(AuthorityKey -> TeacherPermission) { implicit request =>
    val teacherInfo = loggedIn.user.asInstanceOf[Teacher]
