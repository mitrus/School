 ...
 */
object PublicApi extends Controller with MongoController with OptionalAuthElement with AuthConfigImpl {
  ...
  def searchSchool(prefix: String) = AsyncStack { implicit request =>
    ...
    (searchActor ? StartSearch(prefix)).map {
      case SearchFeed(out) => Ok(Json.toJson(out)).as("text/json")
